...
// / scripts/verify-affiliate-system.mjs's findAuthUserByEmail).
...
const merchantB = await findAuthUserByEmail(creds.accounts.merchantB.email)
await findAuthUserByEmail(creds.accounts.renterA.email) // fail fast if the account is missing, even though only its cookie is needed below
