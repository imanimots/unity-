  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="bg-[#8B1A1A] rounded-xl p-8 mb-12 border-l-4 border-l-[#C4511F]">
        <p className="text-[11px] font-medium uppercase tracking-[0.15em] text-white/60 mb-3">Total Commission (all-time)</p>
        <div className="text-5xl lg:text-6xl font-extrabold text-white leading-none mb-2">{money(totalCommission)}</div>
        <p className="text-[11px] font-medium uppercase tracking-[0.12em] text-white/50">Charged to you, the merchant — never added on top of what your customer pays</p>
      </div>

      <div className="mb-12">
        <p className="text-[11px] font-medium uppercase tracking-[0.15em] text-[#9B8B85] mb-4">Transaction History</p>
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-[11px] font-medium uppercase tracking-[0.15em] text-[#9B8B85]">No commissioned transactions yet</p>
          </div>
        ) : (
              ...
              <div key={c.id} className="px-5 py-4 border-b border-[#F2EDE8] dark:border-[#2A1A1A] last:border-b-0">
                <div className="flex items-center justify-between gap-4 mb-2">
                  <div className="min-w-0">
                    <div className="text-sm font-medium text-[#1A0A0A] dark:text-[#F5F0ED] truncate capitalize">
                      {c.listingTitle ?? `${c.transactionType} transaction`}
                    </div>
                    <div className="text-xs text-[#9B8B85]">{formatDate(c.createdAt)}{c.reference ? ` · ${c.reference}` : ''}</div>
                  </div>
                ...
                {c.excessBase > 0 && (
                  <p className="text-[11px] text-[#9B8B85] mt-2">
                    High-value sale: the {(c.standardRateBps / 100).toFixed(1)}% plan rate applied to the first R100,000 ({money(c.standardRateBase, c.currency)}), and 1.0% applied to the remaining {money(c.excessBase, c.currency)}.
                  </p>
                )}
              </div>
            ))}
      ...
      <p className="text-xs text-[#9B8B85] text-center">
        Unity commission and any affiliate reward you&rsquo;ve enabled are deducted from your own proceeds — your customer never pays an added Unity fee.{' '}
        <Link href="/dashboard/merchant/subscription" className="underline hover:text-[#6B5B55] dark:hover:text-[#9B8B85]">Manage your plan</Link>
      </p>
    </div>
  )
}
