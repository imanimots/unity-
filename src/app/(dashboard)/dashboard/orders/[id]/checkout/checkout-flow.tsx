...
const TEST_SCENARIOS = [
  ...
  { value: 'retryable_failure', label: 'Temporary failure (retryable)' },
  { value: 'terminal_failure', label: 'Terminal failure' },
  { value: 'timeout', label: 'Provider timeout / uncertain response' },
] as const
...
export function OrderCheckoutFlow({ orderId, initial }: { orderId: string; initial: InitialSummary }) {
  ...
  const [scenario, setScenario] = useState<(typeof TEST_SCENARIOS)[number]['value']>('success')
  ...
  async function submit() {
    ...
    try {
      ...
      if (!res.ok && res.status !== 200) {
        setError(data.error ?? 'Checkout could not be completed')
    ...
    return (
      <div className="space-y-4">
        <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-900/40 rounded-xl p-5 text-center">
          <p className="text-sm font-semibold text-green-800 dark:text-green-300">Payment complete</p>
          <p className="text-xs text-green-700 dark:text-green-400 mt-1">Order {initial.orderReference} is paid.</p>
        </div>
        <Link
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-[#1A1010] rounded-xl border border-[#F2EDE8] dark:border-[#2A1A1A] p-5 space-y-3">
        <div className="flex justify-between text-sm">
          <span className="text-[#6B5B55] dark:text-[#9B8B85]">Order</span>
          <span className="font-medium text-[#1A0A0A] dark:text-[#F5F0ED]">{initial.orderReference}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-[#6B5B55] dark:text-[#9B8B85]">Item</span>
          <span className="font-medium text-[#1A0A0A] dark:text-[#F5F0ED]">{initial.listingTitle}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-[#6B5B55] dark:text-[#9B8B85]">Seller</span>
          <span className="font-medium text-[#1A0A0A] dark:text-[#F5F0ED]">{initial.sellerDisplayName}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-[#6B5B55] dark:text-[#9B8B85]">Quantity</span>
          <span className="font-medium text-[#1A0A0A] dark:text-[#F5F0ED]">{initial.quantity}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-[#6B5B55] dark:text-[#9B8B85]">Unit price</span>
          <span className="font-medium text-[#1A0A0A] dark:text-[#F5F0ED]">R{initial.unitPrice}</span>
        </div>
        <div className="flex justify-between text-sm">
          <span className="text-[#6B5B55] dark:text-[#9B8B85]">Shipping</span>
          <span className="font-medium text-[#1A0A0A] dark:text-[#F5F0ED]">R{initial.shippingFee}</span>
        </div>
        <div className="flex justify-between text-base pt-3 border-t border-[#F2EDE8] dark:border-[#2A1A1A]">
          <span className="font-semibold text-[#1A0A0A] dark:text-[#F5F0ED]">Total</span>
          <span className="font-bold text-[#1A0A0A] dark:text-[#F5F0ED]">R{initial.totalAmount}</span>
        </div>
      </div>
      ...
      {initial.testModeAvailable && (
        <div>
          <label className="text-xs font-medium text-[#6B5B55] dark:text-[#9B8B85] mb-1 block">Test scenario</label>
          <select
            value={scenario}
      ...
      >
        {submitting ? <><Loader2 size={14} className="animate-spin" /> Processing…</> : `Pay R${initial.totalAmount}`}
      </button>
