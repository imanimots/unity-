...
/**
 * Was entirely mock-mode-gated (404 in real mode) -- now real: checks
 * for an already-open dispute on this booking and redirects to it,
 * otherwise offers the real OpenDisputeDialog scoped to this booking.
 */
export default async function RenterDisputePage({ params }: PageProps) {
  const { id: bookingId } = await params
  const requester = await requireAuth()
  if (!requester) redirect(`/login?redirectTo=/dashboard/renter/bookings/${bookingId}/dispute`)
  ...
  return (
    <div className="min-h-screen bg-[#FAF8F5] dark:bg-[#0F0A0A] px-4 sm:px-6 lg:px-8 py-10">
      <div className="max-w-lg mx-auto">
        <Link href="/dashboard/renter/bookings" className="inline-flex items-center gap-1.5 text-[11px] font-medium uppercase tracking-[0.12em] text-[#9B8B85] hover:text-[#1A0A0A] dark:hover:text-[#F5F0ED] transition-colors mb-8">
