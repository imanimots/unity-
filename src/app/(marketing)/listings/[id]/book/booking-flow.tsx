...
function PriceBreakdownCard({ price }: { price: ReturnType<typeof calculateBookingPrice> | null }) {
  ...
  return (
    <div className="bg-[#FAF8F5] dark:bg-[#1A1010] rounded-xl border border-[#F2EDE8] dark:border-[#2A1A1A] p-5 space-y-3 text-sm">
      <div className="flex justify-between text-[#6B5B55] dark:text-[#9B8B85]">
        <span>
          R{price.rateAmount} × {price.durationDays} day{price.durationDays !== 1 ? 's' : ''}
          {price.rateUnit === 'weekly' && <span className="text-xs"> (weekly rate applied)</span>}
        </span>
      ...
      <p className="text-xs text-[#9B8B85]">This is an estimate — nothing is charged now. Payment is not yet collected on Unity.</p>
    </div>
  )
}
    ...
    <div className="flex items-center gap-2 mb-10">
      {STEPS.filter((s) => s !== 'confirmed').map((step, i) => (
        <div key={step} className="flex items-center gap-2">
          ...
          <span className={`section-label hidden sm:block ${i === idx ? 'text-[#1A0A0A] dark:text-[#F5F0ED]' : ''}`}>{STEP_LABELS[step]}</span>
...
export function BookingFlow({ listing }: { listing: Listing }) {
  ...
  async function submitRequest() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error ?? 'Could not submit your booking request')
        return
      }
  ...
  if (step === 'confirmed') {
    return (
      <div className="max-w-lg mx-auto px-6 py-20 text-center">
        ...
        <h1 className="text-3xl font-extrabold uppercase text-[#1A0A0A] dark:text-[#F5F0ED] mb-3">Request Sent</h1>
        <p className="text-[#6B5B55] dark:text-[#9B8B85] mb-1">
          Your booking reference is <strong className="text-[#1A0A0A] dark:text-[#F5F0ED]">{confirmedReference}</strong>
        </p>
        <p className="text-[#6B5B55] dark:text-[#9B8B85] text-sm mb-10">
          {listing.merchant?.display_name} needs to accept your request before it&apos;s confirmed. You&apos;ll see the status update in your bookings.
        </p>
  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      ...
      <h1 className="text-3xl font-extrabold uppercase text-[#1A0A0A] dark:text-[#F5F0ED] mb-8">Request to Book</h1>

      <StepIndicator current={step} />
      <ListingSummaryCard listing={listing} coverImage={coverImage} />

      {step === 'dates' && (
        <div>
          <div className="flex items-center gap-3 mb-6">
            <Calendar size={18} className="text-[#9B8B85]" />
            <h2 className="text-2xl font-extrabold uppercase text-[#1A0A0A] dark:text-[#F5F0ED]">Select Dates</h2>
          </div>
          ...
          <p className="text-xs text-[#9B8B85] mb-5">
            The merchant may have additional unavailable dates or existing bookings — this is checked when you submit your request.
          </p>
          ...
          {price && (
            <div className="bg-[#FAF8F5] dark:bg-[#1A1010] rounded-xl border border-[#F2EDE8] dark:border-[#2A1A1A] px-5 py-4 mb-5 flex items-center justify-between text-sm">
              <span className="text-[#6B5B55] dark:text-[#9B8B85]">
                ...
                <strong className="text-[#1A0A0A] dark:text-[#F5F0ED]">{price.durationDays} days</strong>
              </span>
      ...
      {step === 'review' && price && (
        <div>
          <h2 className="text-2xl font-extrabold uppercase text-[#1A0A0A] dark:text-[#F5F0ED] mb-6">Review Your Request</h2>
          ...
          <div className="space-y-3 mb-6 text-sm">
            ...
            <div className="flex justify-between items-center px-5 py-4 bg-white dark:bg-[#1A1010] rounded-xl border border-[#F2EDE8] dark:border-[#2A1A1A]">
              ...
              <span className="font-medium text-[#1A0A0A] dark:text-[#F5F0ED]">{price.durationDays} days</span>
            </div>
            <div className="flex justify-between items-center px-5 py-4 bg-white dark:bg-[#1A1010] rounded-xl border border-[#F2EDE8] dark:border-[#2A1A1A]">
          ...
          <label className="block text-xs font-medium text-[#6B5B55] dark:text-[#9B8B85] mb-1.5">Message to merchant (optional)</label>
          <textarea
            value={renterMessage}
            onChange={(e) => setRenterMessage(e.target.value)}
            maxLength={1000}
            rows={3}
            placeholder="Anything the merchant should know about your request..."
          ...
          <div className="flex items-start gap-2 mt-5 mb-6 text-xs text-[#6B5B55] dark:text-[#9B8B85] bg-[#FAF8F5] dark:bg-[#1A1010] rounded-xl p-4 border border-[#F2EDE8] dark:border-[#2A1A1A]">
            ...
            <span>
              This sends a request — the merchant must accept it before it&apos;s confirmed. No payment is collected at this stage.
            </span>
          </div>
          ...
          <label className="flex items-start gap-2.5 mb-6 text-xs text-[#6B5B55] dark:text-[#9B8B85] cursor-pointer">
            ...
            <span>
              I agree to Unity&apos;s{' '}
              <Link href="/rental-terms" className="text-[#8B1A1A] underline hover:no-underline">Rental Terms</Link>,{' '}
              <Link href="/cancellations" className="text-[#8B1A1A] underline hover:no-underline">Cancellation Policy</Link>, and{' '}
              <Link href="/delivery-and-handover" className="text-[#8B1A1A] underline hover:no-underline">Delivery and Handover Terms</Link>.
            </span>
          </label>
          ...
          <div className="flex gap-3">
            ...
            >
              {submitting ? 'Sending…' : 'Send booking request'} <ArrowRight size={16} />
            </button>
          </div>
        </div>
      )}
