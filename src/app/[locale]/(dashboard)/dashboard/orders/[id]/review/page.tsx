...
/**
 * Reviews V2 — Buy/order review (buyer side). Previously dormant: the
 * only prior trace was a dead-end "Leave a review" email CTA pointing at
 * the orders list, with no page behind it. Real, server-backed, using
 * the same submit_review()/ReviewForm authority as every other domain.
 */
export default async function BuyerOrderReviewPage({ params }: Props) {
  const { id } = await params
  const t = await getTranslations('reviews.form')
  const backHref = '/dashboard/orders'
