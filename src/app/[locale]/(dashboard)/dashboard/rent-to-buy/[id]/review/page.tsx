...
/**
 * Reviews V2 — Rent-to-Buy review. Previously dormant: no RTB review
 * path existed at all. One shared page for both customer and merchant
 * (matching RTB's own single unified detail-page convention) —
 * loadReviewPageData resolves the reviewee by comparing the signed-in
 * user against the agreement's own customer_id/merchant_id, not by
 * route. Eligible after ownership transfer (success), or after the
 * required return/settlement lifecycle completes (default/termination
 * following genuine possession) — never for a failure before possession.
 */
export default async function RentToBuyReviewPage({ params }: Props) {
  const { id } = await params
  const t = await getTranslations('reviews.form')
  const backHref = '/dashboard/rent-to-buy'

  const requester = await getRequestProfile()
  const data = requester ? await loadReviewPageData('rent_to_buy', id, requester.userId) : null

  if (!requester || !data) {
    return (
      <div className="min-h-screen bg-[#FAF8F5] dark:bg-[#0F0A0A] flex items-center justify-center">
        <div className="max-w-lg mx-auto px-4 py-16 text-center">
          <p className="text-[#6B5B55] dark:text-[#9B8B85] font-['Plus_Jakarta_Sans']">{t('agreementNotFound')}</p>
    ...
    <div className="min-h-screen bg-[#FAF8F5] dark:bg-[#0F0A0A]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {data.alreadySubmitted && data.existingReview ? (
          ...
          <ReviewForm domain="rent_to_buy" transactionId={id} revieweeName={data.revieweeName} transactionTitle={data.transactionTitle} backHref={backHref} />
        )}
      </div>
    </div>
  )
