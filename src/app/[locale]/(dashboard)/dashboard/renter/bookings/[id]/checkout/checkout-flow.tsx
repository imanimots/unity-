...
function DeadlineCountdown({ paymentDueAt }: { paymentDueAt: string | null }) {
  ...
  if (remainingMs <= 0) {
    return (
      <p className="text-xs text-red-600 dark:text-red-400 flex items-center gap-1.5">
        <Clock3 size={12} /> {t('deadlinePassed')}
      </p>
    )
  }
  ...
  const label = hours > 0
    ? t('timeToPayHours', { hours, minutes })
    : minutes > 0
      ? t('timeToPayMinutes', { minutes, seconds })
      : t('timeToPaySeconds', { seconds })

  return (
    <p className="text-xs text-amber-600 dark:text-amber-400 flex items-center gap-1.5">
      <Clock3 size={12} /> {t('timeToPay', { label })}
    </p>
  )
}
...
export function CheckoutFlow({ bookingId, initial }: { bookingId: string; initial: InitialSummary }) {
  ...
  const refresh = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        setLoadError(data.error ?? t('couldNotLoadStatus'))
        return
      }
      setStatus(data)
      setLoadError(null)
  ...
  async function submit() {
    ...
    try {
      ...
      if (!res.ok && res.status !== 200) {
        setSubmitError(data.error ?? t('couldNotComplete'))
      } else {
        if (data.status === 'late_success_after_expiry') {
          setLateSuccessNotice(data.error ?? t('lateSuccessDefault'))
        }
        try {
      ...
      setSubmitError(t('networkErrorRetry'))
    } finally {
      setSubmitting(false)
    }
  }
  ...
  if (!status) {
    return (
      <div className="flex items-center gap-2 text-sm text-[#9B8B85]">
        <Loader2 size={16} className="animate-spin" /> {t('loadingPaymentDetails')}
      </div>
    )
  }
  ...
  const actionLabel = status.eligibility.allowedActions.includes('retry') ? t('retryPayment') : t('payNow')
  ...
  return (
    <div className="space-y-4">
      ...
      {canAct && (
        ...
        >
          {submitting ? t('processing') : actionLabel}
        </button>
      )}
