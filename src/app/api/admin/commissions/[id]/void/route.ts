...
/** POST /api/admin/commissions/[id]/void -- any non-voided status -> voided. Reason mandatory. */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const gate = await requireAdminForRoute(request, 'admin:commissions:void')
  ...
  if (parsed.data.idempotency_key) {
    ...
    const replay = await checkIdempotentReplay(admin, adminId, 'void_unity_commission', parsed.data.idempotency_key, hash)
  ...
  const { data, error } = await admin.rpc('void_unity_commission', {
    p_actor_type: 'admin',
    p_actor_id: adminId,
  ...
  if (error) {
    console.error('[admin.commissions.void] RPC error', { adminId, commissionId, error })
    const mapped = mapCommissionRpcError(error.message)
    return NextResponse.json({ error: mapped.error }, { status: mapped.status })
  }

  try {
    await notifyMerchantOfUnityCommission(admin, commissionId, 'unity_commission.voided', 'unity-commission-voided', { voidReason: parsed.data.reason })
  } catch (emailErr) {
    console.error('[admin.commissions.void] email dispatch failed', { commissionId, emailErr })
  }
