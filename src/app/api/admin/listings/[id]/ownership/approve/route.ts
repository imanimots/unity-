...
/** POST /api/admin/listings/[id]/ownership/approve -- approveOwnership() via the OwnershipVerificationService abstraction. */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const gate = await requireAdminForRoute(request, 'admin:listings:ownership:approve')
  ...
  try {
    ...
    const result = await provider.approveOwnership(
      { admin },
      listingId,
    ...
    try {
      ...
      if (ctx) {
        await sendTemplate(admin, {
          eventType: 'listing.ownership_approved',
          templateId: 'listing-ownership-approved-merchant',
      ...
      console.error('[admin.listings.ownership.approve] email dispatch failed', { listingId, emailErr })
    }
    ...
    console.error('[admin.listings.ownership.approve] unexpected error', { listingId, err })
    return NextResponse.json({ error: 'Could not approve ownership verification — please try again' }, { status: 500 })
  }
}
