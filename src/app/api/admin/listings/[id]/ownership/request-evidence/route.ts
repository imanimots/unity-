...
/** POST /api/admin/listings/[id]/ownership/request-evidence -- requestAdditionalEvidence() via the OwnershipVerificationService abstraction. */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const gate = await requireAdminForRoute(request, 'admin:listings:ownership:request-evidence')
  ...
  try {
    ...
    const result = await provider.requestAdditionalEvidence(
      { admin },
      listingId,
    ...
    console.error('[admin.listings.ownership.request-evidence] unexpected error', { listingId, err })
    return NextResponse.json({ error: 'Could not request additional evidence — please try again' }, { status: 500 })
  }
}
