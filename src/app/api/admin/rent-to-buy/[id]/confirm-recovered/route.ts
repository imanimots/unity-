...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const gate = await requireAdminForRoute(request, 'admin:rtb:confirm-recovered')
  ...
  const { data, error } = await admin.rpc('confirm_rent_to_buy_recovered', { p_admin_id: gate.requester.userId, p_case_id: parsed.data.case_id, p_idempotency_key: null })
  if (error) {
