...
/**
 * POST /api/admin/reviews/[id]/invalidate — the entire review (rating +
 * text + any reply) is excluded from public display and aggregates.
 * Reserved for genuine invalidity (fraud, duplicate, self-dealing,
 * fabricated eligibility, compromised account, invalid provenance) --
 * never used merely to silence an unwelcome but genuine rating.
 */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const { data, error } = await admin.rpc('admin_invalidate_review', {
  ...
  if (error) {
    console.error('[admin.reviews.invalidate] RPC error', { id, error })
