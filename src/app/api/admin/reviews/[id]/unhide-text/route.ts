...
/** POST /api/admin/reviews/[id]/unhide-text — reverses a prior text-hide moderation action. */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const { data, error } = await admin.rpc('admin_unhide_review_text', {
  ...
  if (error) {
    console.error('[admin.reviews.unhide-text] RPC error', { id, error })
