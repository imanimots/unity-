...
/**
 * POST /api/admin/users/[id]/suspend — cannot create or accept new
 * transactions; protected routes remain available only as needed to
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const gate = await requireAdminForRoute(request, 'admin:users:suspend')
  ...
  if (error) {
    console.error('[admin.users.suspend] RPC error', { userId, error })
