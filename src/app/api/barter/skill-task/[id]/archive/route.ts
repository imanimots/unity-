...
/** POST /api/barter/skill-task/[id]/archive */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const rate = await checkRateLimit(`barter:skill-task:archive:${getClientKey(request)}`, 20, 60_000)
  if (!rate.allowed) {
  ...
  try {
    ...
    return await callSkillTaskOwnerRpc(admin, 'archive_barter_skill_task_post', requester.userId, postId, parsed.data.idempotency_key, 'barter.skill-task.archive')
  } catch (err) {
    console.error('[barter.skill-task.archive] unexpected error', { userId: requester.userId, postId, err })
