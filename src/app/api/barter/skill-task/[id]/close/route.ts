...
/** POST /api/barter/skill-task/[id]/close */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const rate = await checkRateLimit(`barter:skill-task:close:${getClientKey(request)}`, 20, 60_000)
  if (!rate.allowed) {
  ...
  try {
    ...
    return await callSkillTaskOwnerRpc(admin, 'close_barter_skill_task_post', requester.userId, postId, parsed.data.idempotency_key, 'barter.skill-task.close')
  } catch (err) {
    console.error('[barter.skill-task.close] unexpected error', { userId: requester.userId, postId, err })
