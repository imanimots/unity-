...
/** POST /api/barter/skill-task/[id]/pause */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const rate = await checkRateLimit(`barter:skill-task:pause:${getClientKey(request)}`, 20, 60_000)
  if (!rate.allowed) {
  ...
  try {
    ...
    return await callSkillTaskOwnerRpc(admin, 'pause_barter_skill_task_post', requester.userId, postId, parsed.data.idempotency_key, 'barter.skill-task.pause')
  } catch (err) {
    console.error('[barter.skill-task.pause] unexpected error', { userId: requester.userId, postId, err })
