...
/** POST /api/barter/skill-task/[id]/publish */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const rate = await checkRateLimit(`barter:skill-task:publish:${getClientKey(request)}`, 20, 60_000)
  if (!rate.allowed) {
  ...
  try {
    ...
    return await callSkillTaskOwnerRpc(admin, 'publish_barter_skill_task_post', requester.userId, postId, parsed.data.idempotency_key, 'barter.skill-task.publish')
  } catch (err) {
    console.error('[barter.skill-task.publish] unexpected error', { userId: requester.userId, postId, err })
