...
/** POST .../confirm-possession -- a distinct action from payment (Rule 2/E), reachable only once possession_status = 'possession_eligible'. */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const { data, error } = await admin.rpc('confirm_rent_to_buy_possession', { p_actor_user_id: requester.userId, p_agreement_id: id, p_idempotency_key: null })
  if (error) {
