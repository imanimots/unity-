...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const { data, error } = await admin.rpc('decline_rent_to_buy_request', { p_actor_user_id: requester.userId, p_agreement_id: id, p_reason: parsed.data.reason ?? null, p_idempotency_key: null })
  if (error) {
