...
/**
 * POST .../initiate-default -- merchant-facing FORMAL DEFAULT (Rule 17/
 * 18). Irreversible once it succeeds; the RPC itself re-verifies live
 * grace-period eligibility server-side, so this route never trusts a
 * client-computed "is this overdue" signal.
 */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const { data, error } = await admin.rpc('initiate_rent_to_buy_default', {
    p_merchant_id: requester.userId, p_agreement_id: id, p_reason: parsed.data.reason, p_idempotency_key: parsed.data.idempotency_key ?? null,
  })
  if (error) {
  ...
  try {
    ...
    console.error('[rent-to-buy.initiate-default] email dispatch failed', { agreementId: id, emailErr })
  }
