...
import { rentToBuyMutualTerminationProposeSchema } from '@/lib/rent-to-buy/validation'
...
/** POST .../propose-termination -- either party proposes mutual early termination (Rule 19/20). No unilateral effect until the other party accepts. */
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const parsed = rentToBuyMutualTerminationProposeSchema.safeParse(body)
  ...
  const { data, error } = await admin.rpc('propose_rent_to_buy_mutual_termination', {
    p_actor_user_id: requester.userId, p_agreement_id: id, p_reason: parsed.data.reason ?? null, p_idempotency_key: parsed.data.idempotency_key ?? null,
  })
  if (error) {
  ...
  try {
    ...
    if (agreement) {
      ...
      await notifyRentToBuyParty(admin, id, recipientId, 'rent_to_buy.termination_proposed', 'rent-to-buy-termination-proposed', `rtb-termination-proposed-${id}-${Date.now()}`)
    }
  } catch (emailErr) {
    console.error('[rent-to-buy.propose-termination] email dispatch failed', { agreementId: id, emailErr })
  }
