...
import { rentToBuyReturnRequestSchema } from '@/lib/rent-to-buy/validation'
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  const parsed = rentToBuyReturnRequestSchema.safeParse(body)
  ...
  const { data, error } = await admin.rpc('request_rent_to_buy_voluntary_return', {
    p_actor_user_id: requester.userId, p_agreement_id: id, p_condition_notes: parsed.data.condition_notes ?? null, p_idempotency_key: null,
  })
  if (error) {
