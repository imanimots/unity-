...
export function SaleSummaryCard({ listing }: { listing: Listing & { sale_price: number } }) {
  ...
  return (
    <div className="bg-white dark:bg-[#1A1010] rounded-2xl border border-[#F2EDE8] dark:border-[#2A1A1A] p-5 space-y-4 shadow-sm">
      <div className="flex items-center gap-2 text-[#9B8B85]">
        ...
        <span className="text-xs font-medium uppercase tracking-[0.1em]">For sale</span>
      </div>

      <div className="flex items-baseline gap-2">
        <span className="text-5xl font-extrabold text-[#8B1A1A] leading-none">R{listing.sale_price}</span>
      </div>

      {typeof listing.quantity_available === 'number' && (
        <p className="text-sm text-[#6B5B55] dark:text-[#9B8B85]">
          {soldOut ? 'Sold out' : `${listing.quantity_available} available`}
        </p>
      )}
      ...
      <p className="text-xs text-[#9B8B85] text-center flex items-center justify-center gap-1">
        ...
        Secure checkout — test mode, no real money charged
      </p>
    </div>
  )
}
