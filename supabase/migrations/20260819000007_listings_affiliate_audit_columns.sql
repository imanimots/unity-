-- ============================================================
-- Step 11 Phase 7 -- listings affiliate audit columns + server-side
-- commission-rate range enforcement
-- ============================================================
-- accepts_affiliates/affiliate_commission_rate themselves are untouched
-- (already real, already per-listing, already live since the very first
-- schema migration). These 5 nullable columns add the audit trail the
-- spec requires; they are stamped by the new enable_listing_affiliate()/
-- disable_listing_affiliate() RPCs (next migration) when a merchant or
-- admin toggles a published listing -- not by save_listing_draft, which
-- only ever sets the pre-publish initial value.
--
-- affiliate_commission_rate's 1-50 range was previously enforced only
-- client-side (the wizard's zod schema) -- save_listing_draft is the
-- real security boundary (reachable directly, not just via the wizard),
-- so the clamp is added there. Straight CREATE OR REPLACE of the proven
-- function, byte-identical except for the two affiliate_commission_rate
-- expressions (INSERT and UPDATE branches), captured from the live
-- migration source (20260813000004, the latest revision of this
-- function).
...
alter table public.listings
  ...
  add column if not exists affiliate_enabled_by uuid references public.profiles(id),
  add column if not exists affiliate_disabled_at timestamptz,
  add column if not exists affiliate_rate_updated_at timestamptz,
  add column if not exists affiliate_rate_updated_by uuid references public.profiles(id);
