-- ============================================================
-- Phase 3 -- Escrow architecture: overview stats extension (additive,
-- byte-identical to the current live version plus new keys) and
-- email_deliveries entity widening, matching every prior phase's exact
-- pattern.
-- ============================================================
...
alter table public.email_deliveries add constraint email_deliveries_related_entity_type_check check (
  related_entity_type = any (array['booking', 'listing', 'identity_verification', 'order', 'barter_agreement', 'affiliate_commission', 'profile', 'merchant_payout', 'merchant_subscription', 'unity_commission', 'escrow_transaction'])
);
