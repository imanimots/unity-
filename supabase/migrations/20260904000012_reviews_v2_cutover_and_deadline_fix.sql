-- REVIEWS V2 — corrective fix-forward migration.
--
-- Two genuine bugs found during verifier development (20260904000009 is
-- already applied and left untouched; fixed forward here, true supersets
-- of the current live bodies, fetched fresh via pg_get_functiondef
-- immediately before writing this migration):
--
--   1. process_review_deadlines() used max(id)/max(reviewer_id) as
--      aggregates over uuid columns -- Postgres has no default MAX
--      aggregate for uuid, so this raised
--      '42883: function max(uuid) does not exist' on every real call.
--      Replaced with a direct LIMIT 1 subquery for the lone unpublished
--      review, which is what was actually intended.
--
--   2. The v_cutover constant (2026-08-25) was set essentially to
--      migration-apply time, leaving no real elapsed time to construct
--      genuine 'eligible AND already past its 14-day deadline' test
--      fixtures -- violating Reviews V2's own Rule 32 requirement that
--      the cutover be 'deterministic and testable'. Moved to
--      2026-08-01, comfortably more than 14 days before the live
--      now() in this environment (2026-08-26), while still excluding
--      every one of the 37 pre-existing QA-fixture review rows (all
--      created 2026-08-10 through 2026-08-24, i.e. before their own
--      underlying transactions' completion timestamps are considered
--      here) from any retroactive entitlement claim, since none of
--      those rows were ever created through submit_review() in the
--      first place (confirmed in 20260904000008's own audit note) and
--      nothing about this change causes any past transaction to be
--      auto-reviewed.
...
declare
  v_request_hash text;
  v_idem record;
  v_cutover timestamptz := '2026-08-01T00:00:00+00'::timestamptz;
  v_reviewee_id uuid;
