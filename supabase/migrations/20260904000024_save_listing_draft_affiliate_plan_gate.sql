-- P1 remediation: save_listing_draft could set accepts_affiliates=true (on
-- create or edit) with no subscription-plan check at all, while the
-- dedicated enable_listing_affiliate RPC already enforces a Pro/Elite gate
-- (see 20260819072044_subscription_v2_plan_entitlements.sql). Two competing
-- authorities for the same outcome is the actual defect -- this migration
-- makes save_listing_draft enforce the exact same canonical entitlement
-- check (_get_effective_merchant_plan_id + merchant_subscription_plans.
-- affiliate_enabled), reusing the identical helper enable_listing_affiliate
-- already uses, rather than introducing a second independent rule.
--
-- Only a TRANSITION into accepts_affiliates=true is gated (create with
-- true, or edit from false->true) -- re-saving a draft that already has
-- accepts_affiliates=true (a grandfathered Starter listing enabled while
-- previously Pro/Elite) is left untouched, matching the existing
-- grandfathering rule (docs/AFFILIATE_SYSTEM.md) and Part 4's explicit
-- instruction not to invent new downgrade semantics in this phase.
--
-- Parameter list is byte-identical to the live signature (confirmed via
-- pg_get_functiondef immediately before writing this) -- body-only change,
-- so a bare CREATE OR REPLACE is safe (no DROP/second-overload risk).
create or replace function public.save_listing_draft(
  p_listing_id uuid,
  p_listing jsonb,
  p_requirements jsonb,
  p_media jsonb,
  p_category_metadata jsonb default '{}',
  p_private_category_metadata jsonb default '{}',
...
declare
  ...
  v_requested_affiliates boolean;
  v_current_affiliates boolean;
  v_plan_id text;
  v_affiliate_plan_ok boolean;
begin
  ...
  if p_listing_id is null then
    v_requested_affiliates := coalesce((p_listing->>'accepts_affiliates')::boolean, false);
    if v_requested_affiliates then
      v_plan_id := public._get_effective_merchant_plan_id(v_uid);
      select affiliate_enabled into v_affiliate_plan_ok from public.merchant_subscription_plans where id = v_plan_id;
      if not coalesce(v_affiliate_plan_ok, false) then
    ...
    insert into public.listings (
      ...
      v_requested_affiliates,
      least(greatest(coalesce(nullif(p_listing->>'affiliate_commission_rate', '')::numeric, 0), 0), 50),
    ...
    select accepts_affiliates into v_current_affiliates from public.listings where id = v_listing_id;
    v_requested_affiliates := coalesce((p_listing->>'accepts_affiliates')::boolean, v_current_affiliates);

    if v_requested_affiliates and not v_current_affiliates then
      v_plan_id := public._get_effective_merchant_plan_id(v_uid);
      select affiliate_enabled into v_affiliate_plan_ok from public.merchant_subscription_plans where id = v_plan_id;
      if not coalesce(v_affiliate_plan_ok, false) then
        raise exception 'affiliate_requires_pro_or_elite: enabling affiliates requires an active Pro or Elite subscription';
    ...
    update public.listings set
      ...
      accepts_affiliates = v_requested_affiliates,
      affiliate_commission_rate = least(greatest(coalesce(nullif(p_listing->>'affiliate_commission_rate', '')::numeric, affiliate_commission_rate), 0), 50),
